...
def collect_all_datasets(
    ...
    for file_path, data in files_by_size:
        ...
        # Special handling for wiki_ml_corpus - collect more samples
        if "wiki_ml_corpus" in file_path.name.lower():
            # For wiki_ml_corpus, take 50% of available samples (much higher than default 15%)
            actual_samples = min(int(file_size * 1), file_size)
            boost_note = " (boosted 50% for wiki_ml_corpus dataset)"
            
        # Apply boosting strategy based on file size (ordered from smallest to largest)
        elif file_size < 500:
            # Very small files: take 80-90% to ensure strong representation
            boost_percentage = min(0.9, max(0.8, file_size / 500))
            actual_samples = min(int(file_size * boost_percentage), file_size)
            boost_note = f" (boosted {boost_percentage:.0%} for very small file)"
            
        elif file_size < 2000:
            # Small files: take 60-75% to ensure good representation
            boost_percentage = min(0.75, max(0.6, file_size / 2000 * 0.75))
            min_samples = int(file_size * boost_percentage)
            actual_samples = max(base_samples, min_samples)
            actual_samples = min(actual_samples, file_size)
            boost_note = f" (boosted {boost_percentage:.0%} for small file)"
            
        elif file_size < 5000:
            # Medium files: slight boost to maintain representation
            boost_factor = 1.6  # 60% boost
            actual_samples = min(int(base_samples * boost_factor), file_size)
            boost_note = " (60% boost for medium file)"
            
        elif file_size < 10000:
            # Medium-large files: slight reduction (max 40% of file)
            actual_samples = min(base_samples, int(file_size * 0.4))
            boost_note = " (slightly reduced for medium-large file)"
            
        elif file_size < 20000:
            # Large files: moderate reduction (max 25% of file)
            actual_samples = min(base_samples, max_per_file, int(file_size * 0.25))
            boost_note = " (reduced for large file)"
            
        else:
            # Very large files: heavily reduce to prevent dominance (max 15% of file)
            actual_samples = min(base_samples, max_per_file, int(file_size * 0.15))
            boost_note = " (heavily reduced for very large file)"
        
        # Apply absolute maximum limit per file (default: max_per_file samples)
        # For wiki_ml_corpus, allow up to 3x the normal max_per_file limit
        if "wiki_ml_corpus" in file_path.name.lower():
            file_max_limit = min(max_per_file * 3, file_size)
        else:
...
def main():
    ...
    parser.add_argument(
        '--output_dir',
        type=str,
        help='Output directory for collected datasets (default: ../collected_datasets)'
    )
    parser.add_argument(
        '--seed',
        type=int,
    ...
    output_dir = Path(args.output_dir) if args.output_dir else script_dir.parent / "collected_datasets"
